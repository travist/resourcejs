...
describe('Connect to MongoDB', () => {
  it('Create a in memory ReplicaSet', () => MongoMemoryReplSet.create({ replSet: { count: 2 } })
    .then((res) => {
      replSet = res
      uri = replSet.getUri()
    }));

  it('Connect to MongoDB', () => mongoose.connect(uri, {
  ...
  it('Should connect MongoDB without mongoose', () => MongoClient.connect(uri, {
    useUnifiedTopology: true,
    useNewUrlParser: true,
  })
