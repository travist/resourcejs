# Resource.js for KoaJS - A simple library to reflect Mongoose models to a REST interface with automatic OpenAPI3 documentation
...
Resource.js is designed to be a minimalistic library that reflects a Mongoose
model to a RESTful interface. It does this through a very simple and extensible interface.

General idea rolls around middlewares which are added on before and after the mongoose client. Middlewares in order are:

| Middleware | Configrable? | Description | `index()`|`get()`|`post()`|`put()`|`patch()`|`delete()` |
|---|---|---|---|---|
| before | :heavy_check_mark: | Executed before any other before middleware |  |  |
| beforePost | before|
| beforeQuery | :heavy_check_mark: |  |  |  |
| beforeHook | :heavy_check_mark: |  |  |  |
| query | :x: | Executes |  |  |
| afterHook | :heavy_check_mark: |  |  |  |
| afterQuery | :heavy_check_mark: |  |  |  |
...
```
 ...
 - ***app*** - This is the KoaJS application.
