...
ResourceJS supports this feature through a pseudo-method called `virtual`. Given a concrete resource defined by a schema, one or more `virtual` resources can be described by passing `options` to the `resource.virtual` method. The virtual method expects at least the `path` and the `before` parameters to be set.

* name  : set to the name of the virtual resource that will be used in the generated url.
