...
ResourceJS supports this feature through a resource method called `virtual`. Given a resource defined by a schema, one or more `virtual` resources can be described by passing `options` to the `resource.virtual` method. The virtual method expects at least the `path` and the `before` parameters to be set.
